...
describe Findingaids::Solr::CatalogHelpers do
  ...
  describe ".bf_functions" do
    ...
    it { should eql ["exists(query({!v=format_sim:*Collection*}))^1500", "exists(query({!v=level_sim:series}))^300", "exists(query({!v=level_sim:subseries}))^70", "exists(query({!v=level_sim:file}))^50", "exists(query({!v=level_sim:item}))^40"] }
  end
